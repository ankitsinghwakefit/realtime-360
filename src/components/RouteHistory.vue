      ...
      <v-col cols="12" md="4">
        <v-card>
          ...
          <v-card-text>
            ...
            <div>
              <strong>Distance Covered:</strong> {{ distance.toFixed(2) }} km
            </div>
            <div>
              <strong>Average Speed:</strong> {{ avgSpeed.toFixed(2) }} km/h
            </div>
          </v-card-text>
        </v-card>
      </v-col>
...
import { initilizeMap, haversine, getIcon } from "@/utils";

const route = useRoute();
const store = useStore();
const vehicleId = route.params.id;
const vehicle = computed(() =>
  store.getters.getVehicleData.find((v) => v.id === vehicleId)
);
...
onMounted(() => {
  ...
  initilizeMap(
    map,
    vehicle.value.history[0].lat,
    vehicle.value.history[0].lng,
    13
  );
  ...
  L.marker([start.lat, start.lng], { icon: getIcon("Online") })
    .addTo(map)
    .bindPopup("Start")
    .openPopup();
  L.marker([end.lat, end.lng], { icon: getIcon("Alert") })
    .addTo(map)
    .bindPopup("End");
  ...
  const endTime = new Date(
    vehicle.value.history[vehicle.value.history.length - 1].timestamp
  );
